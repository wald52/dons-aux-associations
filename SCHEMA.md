...
| `beneficiary_kind_provenance` | enum | `declared` si la source publie la nature juridique du bénéficiaire, `guessed` si nous l'avons déduite du nom. Une nature **déclarée** hors du champ associatif sort la ligne des totaux ; une nature **devinée** ne suffit pas. L'asymétrie est voulue : exclure à tort efface une association réelle, inclure à tort laisse une ligne visible et corrigeable. **Depuis la phase 15, cette asymétrie ne joue plus que là où aucun registre ne tranche** — voir les quatre colonnes ci-dessous. |
| `beneficiary_legal_category` | int32, nullable | Catégorie juridique INSEE du bénéficiaire (`9220` association déclarée, `9230` reconnue d'utilité publique, `9223` groupement d'employeurs, `9300` fondation ou fonds de dotation, `73xx` personne morale de droit public, `5xxx` société…). Posée par `enrich_nature.py` depuis `data/referentiel/nature-beneficiaires.parquet`, extrait de SIRENE. **NULL quand le bénéficiaire n'a pas de SIREN exploitable** — 23,1 % du montant. |
| `beneficiary_is_associatif` | bool, nullable | Le verdict, dérivé de la colonne précédente par `est_associatif` : vrai pour `92xx` et `9300`, faux sinon, **NULL quand on ne sait pas**. `NULL n'est pas un « non »` : la ligne reste comptée. C'est le seul champ que la règle des totaux consulte. |
| `beneficiary_family` | string | La famille **telle qu'elle est affichée** sous le nom sur la fiche : « association déclarée », « fondation ou fondation d'entreprise », « groupement d'employeurs », « nature non vérifiée », « hors périmètre — ni association ni fondation »… Treize valeurs. Elle tient la consigne de différenciation : un total unique mêlant une association de quartier et un fonds de dotation d'entreprise serait exact et trompeur. |
| `beneficiary_type_jo` | string, nullable | Le type **déclaré par l'organisme lui-même** au dépôt de ses comptes au Journal officiel : « Associations loi du 1er juillet 1901 », « Fonds de dotation », « Fondations-Fondations d'entreprise », « Fondations partenariales ». Il affine `beneficiary_family` à l'intérieur du code `9300`, que l'INSEE confond, mais **ne renverse jamais** le verdict : les deux registres divergent sur 92 organismes (240,2 M€) et aucun ne ment — l'INSEE enregistre la FORME JURIDIQUE, le JO le TYPE DÉCLARÉ. |
